...
function getTransporter(): nodemailer.Transporter {
  ...
  _transporter = nodemailer.createTransport({
    ...
    // Prevent stale connection timeouts
    connectionTimeout: 10_000, // 10s to establish connection
    greetingTimeout: 5_000, // 5s for server greeting
    socketTimeout: 15_000, // 15s per socket operation
    // Connection pool — reuse connections, close idle ones
    pool: true,
    maxConnections: 3,
    maxMessages: 100,
    // TLS — accept self-signed certs in dev, strict in production
    tls: {
