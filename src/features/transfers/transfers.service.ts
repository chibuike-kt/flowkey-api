/**
 * FlowKey — Transfers Service
 *
 * All money movement in one place:
 *   FlowKey → FlowKey  — instant settlement, double-entry ledger
 *   FlowKey → Bank     — async lifecycle, debit immediate, credit via processor
 *
 * UID auth: user authenticates with their own UID + PIN on any device.
 * The sender is always the UID owner, never the device session holder.
 *
 * Invariants:
 *   Pessimistic row lock before every balance read
 *   Every transfer produces exactly 2 ledger entries (double-entry)
 *   Idempotency key checked before any DB write
 *   Fraud check before any ledger operation
 *   Full audit chain on every operation
 *   Auto-reversal on bank transfer failure
 */
...
export async function executeInternalTransfer(
  ...
  if (!senderData.user_auth?.transaction_pin_hash) throw new AppError(ErrorCode.CONFLICT, 'Please set a transaction PIN first.');

  assertActive(senderData.account_status, 'Your');

  const senderUserId   = deviceUserId;
  const senderWalletId = senderData.wallet.id;
  const senderAuth     = senderData.user_auth;
...
export async function initiateBankTransfer(
  ...
  if (!sender.user_auth?.transaction_pin_hash) throw new AppError(ErrorCode.CONFLICT, 'Please set a transaction PIN first.');
