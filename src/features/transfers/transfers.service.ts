...
export async function executeInternalTransfer(
  ...
  if (!senderData.auth?.transaction_pin_hash)
...
export async function initiateBankTransfer(
  ...
  if (!sender.auth?.transaction_pin_hash)
    throw new AppError(ErrorCode.CONFLICT, 'Please set a transaction PIN first.');
...
async function loadUidSender(universalId: string): Promise<{
  ...
  if (!user.auth?.upp_hash) {
    throw new AppError(
      ErrorCode.CONFLICT,
      'Universal Payment PIN not set. Set your UPP in the FlowKey app before using UID transfers.',
    );
  }
...
export async function executeUidInternalTransfer(
  ...
  return {
    ...
    recipient: {
      ...
      display_name:
        uidRecipientWallet?.user?.display_name ?? uidRecipientWallet?.user?.username ?? '',
